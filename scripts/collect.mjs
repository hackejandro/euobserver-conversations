...
const isEuobserver = (value) => {
  try {
    const hostname = new URL(value).hostname.toLowerCase().replace(/^www\./, '');
    return hostname === 'euobserver.com' || hostname.endsWith('.euobserver.com');
  } catch { return false; }
};
...
function linksFrom(record = {}) {
  ...
  for (const link of found) {
    ...
    if (url && isEuobserver(url) && !unique.has(url)) unique.set(url, { ...link, url });
  }
...
function collectReplies(node, replies) {
  for (const child of node?.replies || []) {
    if (child?.post) replies.push(replyFrom(child.post));
...
async function loadState() {
  ...
  catch { return { cursor: 0, candidates: [], featured: [] }; }
}
...
async function buildFeed(state) {
  ...
  for (const url of ordered) {
    ...
    if (!hydrated.some((post) => (post.replies || []).length > 0)) continue;
